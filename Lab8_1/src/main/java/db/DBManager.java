...
public class DBManager {
	...
	public DBManager() {
		try {
			connection = DriverManager.getConnection("jdbc:postgresql://localhost:5432/Lab8_1?user=postgres&password=1234");
	...
	public File getFile(String name) throws DBException {
		try{
			PreparedStatement preparedStatement = connection.prepareStatement("SELECT * FROM files WHERE name = ?");
	...
	public Folder getFolder(String name) throws DBException {
		try{
			PreparedStatement preparedStatement = connection.prepareStatement("SELECT * FROM folders WHERE name = ?");
			preparedStatement.setString(1, name);
			ResultSet resultSet = preparedStatement.executeQuery();
			if (resultSet.next()){
	...
	public File insertFile(File file) throws DBException {
		if (file == null)
			return null;
		try{
	...
	public File insertFileToFolder(File file, Folder folder) throws DBException {
		file.setFolderId(getFolder(folder.getName()).getId());
		return insertFile(file);
	}

	public boolean transferFile(File file, Folder folder) throws DBException {
		try {
			file.setId(getFile(file.getName()).getId());
			file.setFolderId(getFolder(folder.getName()).getId());
	...
	public Folder insertFolder(Folder folder) throws DBException {
		if (folder == null)
			return null;
		try {
	...
	public boolean updateFile(File file) throws DBException {
		try {
			file.setId(getFile(file.getName()).getId());
			file.setFolderId(getFile(file.getName()).getFolderId());
			PreparedStatement statement = connection.prepareStatement("UPDATE files SET name=?, size=?, folder_id=? WHERE id=?");
			statement.setString(1, file.getName());
			statement.setInt(2, file.getSize());
			statement.setInt(3, file.getFolderId());
	...
	public boolean deleteFile(Folder folder, File file) throws DBException {
		try {
			PreparedStatement statement = connection.prepareStatement("DELETE FROM files WHERE name=? AND size=? AND folder_id=?");
			statement.setString(1, file.getName());
			statement.setInt(2, file.getSize());
			statement.setInt(3, getFolder(folder.getName()).getId());
			statement.executeUpdate();
			return true;
		} catch (SQLException e) {
			throw new RuntimeException(e);
	...
	public List<File> getFilesInFolder(Folder folder) throws DBException {
		try{
			PreparedStatement preparedStatement = connection.prepareStatement("SELECT * " +
					"FROM files " +
					"WHERE folder_id=?");
			preparedStatement.setInt(1, getFolder(folder.getName()).getId());
			ResultSet resultSet = preparedStatement.executeQuery();
			List<File> filesList = new ArrayList<>();
			while (resultSet.next()){
